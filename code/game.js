...
function Moving_Platform (pos, ch){
	this.pos = pos;
	this.size = new Vector(1,1);
	if (ch == "w"){
		this.speed = new Vector(0,2);
...
Moving_Platform.prototype.type = "lift";

function Nega_Warp (pos, ch){
	this.pos = pos;
	this.size = new Vector (1,1);
	if (ch == "e"){
...
Nega_Warp.prototype.type = "nwarp";

function Slow_down(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.4, 0.4);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Slow_down.prototype.type = "slow";
...
Moving_Platform.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size)){
    this.pos = newPos;
} else if (this.repeatPos)
...
Nega_Warp.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size)){
    this.pos = newPos;
} else if (this.repeatPos)
...
Slow_down.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        startLevel(n);
			//New area of code
	else if (status == "warp" && status != "praw")
		startLevel (n + 1);
	//End of new area of code
	else if (status == "praw" && status != "warp")
		startLevel (n -= 1);
